...
function makeCubicDeadbandFilter(weight, deadband) {
  ...
  return (x) => {
    ...
    return (
      (weight * Math.pow(x, 3) + iw * x - (Math.abs(x) / x) * scaleFactor) /
      (1 - scaleFactor)
    );
  };
}
...
export class SmoothingDeadbandFilter {
  ...
  constructor({
    ...
    this._deadbandFilter = makeCubicDeadbandFilter(
      cubicDeadbandWeight,
      deadband,
    );
  ...
  process(transIn, rotIn) {
    let transProcessed = this._processComponent(
      transIn,
      this._previousOutput[0],
      this.translationMultiplier,
    );
    let rotProcessed = this._processComponent(
      rotIn,
  ...
  _processComponent(v, vPrev, scaleFactor = 1.0) {
    ...
    if (filteredNorm === 0.0) {
      ...
      const filteredExp = filtered.map((x) =>
        Math.exp(Math.abs(x) / this.softmaxTemperature),
      );
      ...
      filtered = filtered.map(
        (x) => x * (filteredNorm / softmaxedNorm) * scaleFactor,
      );
      // Combine the filtered value with the previous value for smoothing
      filtered = filtered.map(
        (x, i) => (1 - this.smoothing) * x + vPrev[i] * this.smoothing,
      );
    }
    return filtered;
  }

...
export class ThreeDMouse {
  ...
  constructor(
    device,
    dataSpecs,
    { emitRepeatedEvents: emitRepeatedEvents = false, filter: filter },
  ) {
    this.device = device;
  ...
  set emitRepeatedEvents(value) {
    ...
    if (this._emitRepeatedEvents) {
      this._emitRepeatedEventsInterval = window.setInterval(
        this._emitRepeatedEvent.bind(this),
        1,
      );
    } else {
      window.clearInterval(this._emitRepeatedEventsInterval);
  ...
  static async requestDevice(options) {
    ...
    if (!this.device.opened) {
      ...
      const driverName =
        IDS_TO_NAME[[this.device.vendorId, this.device.productId]];
  ...
  handleInputReport(e) {
    ...
    for (const [
      name,
      { channel: chan, byte: byte, scale: flip },
    ] of Object.entries(this.dataSpecs.mappings)) {
      if (e.reportId === chan) {
    ...
    for (
      let button_index = 0;
      button_index < this.dataSpecs.buttonMapping.length;
      button_index++
    ) {
      const {
        name: name,

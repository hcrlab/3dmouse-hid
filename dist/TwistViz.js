...
export class TwistViz {
  ...
  constructor({
    ...
    this._resizeObserver = new ResizeObserver(
      this._elementResized.bind(this),
    ).observe(container);
  }
  ...
  setTwist(twist) {
    ...
    marker.rotation.copy(
      new THREE.Euler().setFromVector3(ensureVector3(twist[1])),
    );
  }
  ...
  _elementResized(_) {
    this.renderer.setSize(
      this.container.clientWidth * this.scale,
      this.container.clientHeight * this.scale,
    );
    this.camera.aspect =
  ...
  setCameraToInitialState() {
    this.camera.position.set(-0.5, -3, 3);
